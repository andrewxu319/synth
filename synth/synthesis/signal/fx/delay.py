    ...
    def __next__(self):
        ...
        if self.delay_time > 0:
            if self.chunks_elapsed < self.chunks_to_wait: # havent reached point to start delay signal yet
                ...
                mix[:self.frames_into_chunk] += self.next_chunk_start_addition #WORK ON THIS TMRW
                # print(mix[:self.frames_into_chunk])
                # print(self.next_chunk_start_addition)
                mix[self.frames_into_chunk:] += self.delay_buffer[:self.frames_per_chunk - self.frames_into_chunk]
